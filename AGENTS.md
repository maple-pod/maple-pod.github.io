...
2. **Discover semantic state:** `pnpm exec spec graph export --root .` gives normalized Story / Feature / Rule / Scenario / Contract / Clause nodes and their `motivates`, `demonstrates` and `constrains` edges. Use `spec graph get/list/incoming/outgoing` and the six semantic commands (`story`, `feature`, `rule`, `scenario`, `contract`, `clause`) to inspect or change specific IDs rather than guessing references. The old `spec resource` commands do not exist in v1.
...
5. **Verify changes:** finish every mutation sequence with `pnpm exec spec workspace validate --root .` and `pnpm exec spec graph export --root .`, plus relevant code lint/type-check/build/behavioral checks. Validation proves structural/reference invariants only, not prose adequacy or implementation conformance. Review any authority-changing edit explicitly. Run `pnpm spec:scenario-quality` to reject duplicated Gherkin step prefixes. `pnpm spec:migration-check` independently replays the immutable historical migration and never compares the evolving current `.spec/` with old source prose; `pnpm spec:baseline-compare` is a one-time premerge check and is **not** a routine quality gate.
