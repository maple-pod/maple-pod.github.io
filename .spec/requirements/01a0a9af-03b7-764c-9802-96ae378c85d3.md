...
Observed in `src/router/index.ts`, `src/utils/common.ts`, `src/schemas/SaveablePlaylist.ts`, `src/schemas/HashActionImportSaveablePlaylistData.ts`, and share-link UI code. The transport protocol, semantic payload fields, shareable playlist kinds, prefilled creation flow, and unresolved-music disclosure/consent behavior have been explicitly reviewed and accepted. Current implementation validates payload shape but does not yet resolve every imported music ID or implement the reduced-import consent branch; this accepted requirement therefore exposes an implementation gap rather than documenting current behavior.
